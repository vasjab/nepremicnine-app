...
const STORAGE_KEY = 'checklist-expanded';
...
export function FormCompletionChecklist({ items, title }: FormCompletionChecklistProps) {
  ...
  const { 
    ...
    const totalItems = required.length + bonus.length;
    const totalCompleted = reqCompleted + bonCompleted;
    ...
    <Card className="sticky top-20 z-10 mb-6 border-border/50 bg-card/95 backdrop-blur-md shadow-md">
      <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors rounded-t-lg">
            <div className="flex items-center gap-3">
              ...
              <Progress 
                ...
                className={cn(
                  "w-24 sm:w-32 h-2",
                  isAllRequiredComplete && bonusPercentage >= 75 && "[&>div]:bg-amber-500",
                  isAllRequiredComplete && bonusPercentage < 75 && "[&>div]:bg-green-500"
                )}
              />
            ...
            <div className="flex items-center gap-3 sm:gap-4 text-sm">
              ...
              <span className={cn(
                ...
                {isAllRequiredComplete && <Check className="h-3 w-3 inline ml-1" />}
              </span>
              {bonusItems.length > 0 && (
